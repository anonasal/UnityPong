...
public class Ball : MonoBehaviour
{
    ...
    [Tooltip("The max ball angle after colliding with a paddle")]
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (wall) 
        {
            ...
            EmitParticle(8);
            GameManager.instance.screenshake.StartShake(0.05f, 0.033f);
        }
    }

    private void AdjustAngle(Paddle paddle, Collision2D collision)
    {
        //ball move at higher angle toward the edge of the paddle
        Vector2 median = Vector2.zero;
        foreach (ContactPoint2D point in collision.contacts)
        {
            median += point.point;
            // Debug.DrawRay(point.point, Vector3.right, Color.cyan, 1f);
        }
        median /= collision.contactCount;
        // Debug.DrawRay(median, Vector3.right, Color.red, 1f);
        
        // calculates relative distance from center (between -1 and 1)
